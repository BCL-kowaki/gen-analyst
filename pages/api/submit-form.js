...
async function submitToProline(uid, name, phone) {
  try {
    ...
    formData.append("txt[wSpDEb629H]", name);
    formData.append("txt[txnzkcjsDt]", phone);

    console.log("=== プロライン送信情報 ===");
    console.log("UID:", uid || "未設定");
    console.log("名前:", name);
    console.log("電話番号:", phone);
    console.log("送信先URL: https://z8nhy9aq.autosns.app/fm/s0hmTUfxp3");
    console.log("========================");

    const response = await fetch("https://z8nhy9aq.autosns.app/fm/s0hmTUfxp3", {
      method: "POST",
      headers: {
